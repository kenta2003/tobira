...
### 個人的な動機
長期インターンでサイト実装・LP 制作・SEO 対策などを業務として担当してきた中で、与えられた範囲を実装するだけでなく「**サービス構想 → 設計 → 実装 → 集客・運用までを一人で一気通貫でやり切れるのか**」を自分自身で検証したかった。インターン業務では分業されていた領域（プロダクト設計 / バックエンド / 決済 / SNS 運用 / SEO / 情報の継続的な品質担保）を、自発的に立ち上げた個人プロジェクト上で全て統合的に実装することを目的としている。
