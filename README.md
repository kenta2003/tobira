...
### 個人的な動機
長期インターンでサイト実装・LP 制作・SEO 対策などのプロジェクトを任せていただく中で、**今後より主導的にプロジェクトを動かしていく上で、自分の中での各工程の解像度を上げたい**という思いがあった。インターンでは分業されている領域（プロダクト設計 / バックエンド / 決済 / SNS 運用 / SEO / 情報の継続的な品質担保）を、サービス構想 → 設計 → 実装 → 集客・運用まで一人で一気通貫でやり切ることで統合的に経験するため、自発的に個人プロジェクトを立ち上げた。
