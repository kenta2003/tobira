...
地域・季節・カテゴリで日本各地の観光スポットを探せる、地図ベースの観光ガイド Web アプリケーション。Stripe 決済によるサブスクリプション、エリア別ブログ記事、Instagram カルーセル投稿の半自動生成機能を含む、フロントエンドからバックエンド・運用ツールまで一気通貫のプロダクト。

> **本プロジェクトは個人開発によるポートフォリオです。サービス自体は現在停止しています。ローカル環境（`npm run dev`）での動作確認が可能です。**

デモ動画: [`docs/demo.mp4`](docs/demo.mp4)
...
### 個人的な動機
長期インターンでサイト実装・LP 制作・SEO 対策などのプロジェクトを任せていただく中で、与えられた要件を実装することはできていたものの、「**自分は本当にゼロからサービスを立ち上げられるのか**」「**どの工程が得意で、どこで詰まるのか**」といった自己理解が曖昧なままだった。これを言語化するために、サービス構想 → 設計 → 実装 → 集客・運用までを一人で一気通貫でやり切る個人プロジェクトを自発的に立ち上げ、インターンでは分業されていた領域（プロダクト設計 / バックエンド / 決済 / SNS 運用 / SEO / 情報の継続的な品質担保）を統合的に実装した。

### 取り組んだ市場課題
日本の既存観光メディアは「ランキング順の羅列」や「行ったことのある人だけが知っている情報」に偏っており、旅行者が自分の関心（季節・カテゴリ・移動可能エリア）に合うスポットへ辿り着くまでのコストが高い、という課題があった。

### 目的
「地図 × 多軸フィルタ × 一次情報ベースの情報品質」を軸に、旅行者が30秒以内に行きたいスポットを発見できる体験を構築する。閲覧体験だけでなく、運営側（情報の継続更新、SNS による集客、SEO 対応）まで含めて設計する。
...
- LLM を使わずテンプレートエンジンのみで成立させることで、生成コストをゼロに抑えた
